...
public class AttackRangeHighlightAbility : Ability
{
    ...
    public override void OnCellSelected(Cell cell, CellGrid cellGrid)
    {
        ...
        var availableDestinations = UnitReference.GetComponent<TbsFramework.Units.Abilities.MoveAbility>().availableDestinations;
        if (!availableDestinations.Contains(cell)) { return; }
    ...
    public override void OnAbilityDeselected(CellGrid cellGrid) { inRange?.ForEach(u => u.UnMark()); }

    public override void OnTurnEnd(CellGrid cellGrid) { inRange = null; }
}