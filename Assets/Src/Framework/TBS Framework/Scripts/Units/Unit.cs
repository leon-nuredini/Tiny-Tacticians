...
namespace TbsFramework.Units
{
    ...
    public class Unit : MonoBehaviour
    {
        ...
        public void RegisterAbility(Ability ability)
        {
            ...
            if (AbilityAddded != null) { AbilityAddded.Invoke(this, new AbilityAddedEventArgs(ability)); }
        }
        ...
        public virtual void SetState(UnitState state) { UnitState.MakeTransition(state); }

        /// <summary>
        /// A list of buffs that are applied to the unit.
        /// </summary>
        protected List<(Buff buff, int timeLeft)> Buffs;
        ...
        public float TotalMovementPoints { get; protected set; }
        public float TotalActionPoints   { get; protected set; }
        public int   Priority;

        /// <summary>
        /// Cell that the unit is currently occupying.
        /// </summary>
        [SerializeField] [HideInInspector] private Cell cell;

        public Cell Cell { get { return cell; } set { cell = value; } }

        [BoxGroup("Stats")] private int _hitPoints;
        ...
        public int HitPoints
        {
            ...
            {
                _hitPoints = value;
                OnHitpointsChange?.Invoke();
            }
        }

        [BoxGroup("Stats")] [HideInInspector] public int AttackRange;
        [BoxGroup("Stats")] [HideInInspector] public int AttackFactor;

        /// <summary>
        /// Determines how far on the grid the unit can move.
        /// </summary>
        [BoxGroup("Stats")] private float movementPoints;
        ...
        {
            if (UnitClicked != null) { UnitClicked.Invoke(this, EventArgs.Empty); }
        }

        protected virtual void OnMouseEnter()
        {
            if (UnitHighlighted != null) { UnitHighlighted.Invoke(this, EventArgs.Empty); }
        }

        protected virtual void OnMouseExit()
        {
            if (UnitDehighlighted != null) { UnitDehighlighted.Invoke(this, EventArgs.Empty); }
        }
        ...
        public virtual void OnUnitSelected()
        {
            ...
            if (UnitSelected != null) { UnitSelected.Invoke(this, EventArgs.Empty); }
        }
        ...
        public virtual void OnUnitDeselected()
        {
            ...
            if (UnitDeselected != null) { UnitDeselected.Invoke(this, EventArgs.Empty); }
        }
        ...
        protected virtual void AttackActionPerformed(float actionCost) { ActionPoints -= actionCost; }
        ...
        {
            return Mathf.Clamp(damage, 1, damage);
        }
        ...
        public virtual IEnumerator Move(Cell destinationCell, IList<Cell> path)
        {
            ...
            if (MovementAnimationSpeed > 0) { yield return MovementAnimation(path); }
            else
            {
        ...
        protected virtual IEnumerator MovementAnimation(IList<Cell> path)
        {
            for (int i = path.Count - 1; i >= 0; i--)
            {
                ...
                Vector3 destination_pos = FindObjectOfType<CellGrid>().Is2D
                    ? new Vector3(currentCell.transform.localPosition.x,
                                  currentCell.transform.localPosition.y,
                                  transform.localPosition.z)
                    : new Vector3(currentCell.transform.localPosition.x,
                                  currentCell.transform.localPosition.y,
                                  currentCell.transform.localPosition.z);
        ...
        public virtual void UnMark()
        {
            if (UnitHighlighterAggregator != null)
            {
                ...
                IsUnitReachable = false;
            }
        }

        public void SetMovementPoints(float movementPoints) => MovementPoints = movementPoints;
        ...
        private void Reset()
        {
            if (GetComponent<Abilities.AttackAbility>() == null) { gameObject.AddComponent<Abilities.AttackAbility>(); }

            if (GetComponent<Abilities.MoveAbility>() == null) { gameObject.AddComponent<Abilities.MoveAbility>(); }
    ...
    public class AbilityAddedEventArgs : EventArgs
    {
        ...
        public AbilityAddedEventArgs(Ability ability) { this.ability = ability; }
    }
}